...
class Bird:
    IMGS = BIRD_IMGS
    MAX_ROTATIONS = 25  # Max angle to which the bird can tilt
    ROT_VEL = 20 # how the bird rotates each frame
    ANIMATION_TIME = 5 # how long each bird animation will be shown
...
def main(genomes, config):
    ...
    while run and len(birds) > 0:
        ...
        for x,bird in enumerate(birds):
            if bird.y + bird.img.get_height() - 10 >= 600 or bird.y < -50:
                birds.pop(x)
                nets.pop(x)
                ge.pop(x)
